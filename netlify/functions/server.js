...
// File paths for counters (ajustado para Netlify)
const FILES = {
  chamado: path.join(__dirname, 'contador.json'),
  ticket: path.join(__dirname, 'contadort.json'),
  devolucao: path.join(__dirname, 'contadordev.json'),
...
const updateCount = (file, value) => {
  try {
    fs.writeFileSync(file, JSON.stringify({ total: value }, null, 2));
  } catch (error) {
...
const readUsers = () => {
  try {
    return JSON.parse(fs.readFileSync(FILES.users, 'utf8'));
  } catch {
    return { users: [] };
  }
};
...
const saveUsers = (data) => {
  try {
    fs.writeFileSync(FILES.users, JSON.stringify(data, null, 2));
  } catch (error) {
...
const readReports = () => {
  try {
    return JSON.parse(fs.readFileSync(FILES.reports, 'utf8'));
  } catch {
    return { reports: [] };
  }
};
...
const saveReports = (data) => {
  try {
    fs.writeFileSync(FILES.reports, JSON.stringify(data, null, 2));
  } catch (error) {
